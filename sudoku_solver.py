...
def hidden_singles_rows_cols(arr, poss):
    for r in range(9):
        for num in range(1, 10):
            possible_cols = []
            for c in range(9):
                if arr[r][c] == 0 and num in poss[r][c]:
...
def constraint_propagation(arr):
    """Apply all constraint propagation techniques until no more progress"""
    poss = [[[] for _ in range(9)] for _ in range(9)]
    while True:
        num_possible(arr, poss)
        
        if naked_singles(arr, poss):
            continue
        if singles_method(arr, poss):
            continue
        if hidden_singles_rows_cols(arr, poss):
            continue
...
def is_valid(arr):
    ...
    # Check if any empty cell has no possibilities
    for r in range(9):
        for c in range(9):
            if arr[r][c] == 0 and len(poss[r][c]) == 0:
                return False
    return True

def find_best_guess_cell(arr, poss):
    """Find empty cell with fewest possibilities (good heuristic)"""
    best_cell = None
...
def solve_with_backtracking(arr, depth=0):
    ...
    for num in possibilities:
        ...
        # Make a copy and try this guess
...
for row in puzzle:
    print(row)
print(f"Zeros remaining: {count_zeros(puzzle)}")